...
Route::action('install_exists', function () {
        if (file_exists('install') && !Session::get('messages.error')) {
            Notify::error(['<h4 class="alert-heading"> Important Security Notification!</h4> Please remove the install directory before publishing your site.<hr><button type="button" id="deleteInstall" class="btn btn-danger w-100">Delete Install</button>']);
        }
  });

Route::post('admin/install/delete', function () {
    ...
    $installFolder = PATH . 'install'; // Adjust the path to your install folder
    $success = deleteFolder($installFolder);

    if ($success) {
        return Response::json([
            'success' => true,
            'message' => 'Installation folder removed successfully.'
        ]);
    } else {
        return Response::json([
            'success' => false,
            'message' => 'Failed to remove installation folder.'
        ]);
    }
});
