...
/**
 * Return the number of the article based on the author ID
 *
 * @param int $author_id The ID of the author (optional)
 * @return int The article number for the author
 * @throws \Exception
 */
function article_number($author_id = null)
{
    // If no author ID is provided, use the current article's author ID
    if (is_null($author_id)) {
        $author_id = article_author_id();
    }

    // Calculate the article number based on the author ID
    $article_number = Post::where(Base::table('posts.status'), '=', 'published')
        ->where(Base::table('posts.author'), '=', $author_id)
        ->where(Base::table('posts.id'), '<=', article_id())
        ->count();

    // Ensure the article number is at least 1
    return max(1, $article_number);
}
...
function cover() {
    $image = Registry::prop('article', 'image');
    $videolink = Registry::prop('article', 'videolink');
    $imagelink = Registry::prop('article', 'imagelink');
    $content = '';

    // Debugging output to check what is being retrieved
    error_log("Debug: Image = $image, Video link = $videolink, Image link = $imagelink");

    if (!empty($image)) {
        $content .= '<img src="' . $image . '" alt="Article Image" style="max-width:100%;height:auto;">';
    }

    if (!empty($videolink)) {
        $embedCode = generateVideoIframe($videolink);
        if ($embedCode !== 'Unsupported video link') {
            $content .= $embedCode;
        } else {
            error_log("Unsupported video URL provided.");
        }
    }

    if (!empty($imagelink)) {
        $content .= '<img src="' . $imagelink . '" alt="Additional Image" style="max-width:100%;height:auto;">';
    }

    if (empty($content)) {
        $base_url = base_url('assets/img/no_img.png'); // Default no-image link
        $content = '<img src="' . $base_url . '" alt="No Image Available" style="max-width:100%;height:auto;">';
    }
...
function article_author_avatar()
{
    ...
    if ($hasimage) {
        ...
        $image = base_url('assets/img/no_avatar.png');
    }
    return $image;
}
