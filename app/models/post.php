...
class post extends Base
{
        ...
        return static::left_join(
            ...
            Base::table('posts.author')
                     ...
                     ->fetch([
                         ...
                         Base::table('users.image as author_avatar')
                     ]);
    }
    ...
    /**
     * Retrieves the amount of views for a post
     *
     * @param \post|\stdClass $post post to retrieve the views amount for
     *
     * @return int number of views the post received
     * @throws \Exception
     */

    public static function getViewsCount($post) {
        // Check if $post is an object and get the ID, or assume it's already an ID
        $postId = is_object($post) ? $post->id : $post;

        return (int)Query::table(Base::table('views'))
            ->where('post', '=', $postId)
            ->count();
    }
    /**
     * Retrieves the amount of likes for a post
     *
     * @param \post|\stdClass $post post to retrieve the likes amount for
     *
     * @return int number of likes the post received
     * @throws \Exception
     */
    public static function getLikesCount($post) {
        $postId = is_object($post) ? $post->id : $post;

        return (int)Query::table(Base::table('likes'))
            ->where('post', '=', $postId)
            ->count();
    }
    ...
    public static function getCommentsCount($post) {
        ...
        return (int)Query::table(Base::table('comments'))
            ->where('post', '=', $postId)
            ->count();
    }
    ...
    public static function search($term, $page = 1, $per_page = 10)
    {
        ...
        $query = static::left_join(
            ...
            Base::table('posts.author')
                       ...
                       ->or_where(Base::table('posts.description'), 'like', '%' . $term . '%');
