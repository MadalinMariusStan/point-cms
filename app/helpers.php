...
function asset_admin_url($extra = '')
{
    return asset('/' . ltrim($extra, '/'));
}
...
function avatar()
{
    ...
    if ($user->image):
        ...
        $avatar = Uri::to('app/views/assets/img/no_avatar.png');
    endif;
...
function countOnlineVisitors() {
    ...
    // Count the number of visitors active within the threshold
    $count = Query::table(Base::table('visitors'))->where('updated_at', '>=', $threshold)->count();

    return $count;
}

function getAuthorRealName($userId) {
    // Query the 'users' table to get the real name of the author based on the user ID
    $author = Query::table(Base::table('users'))
        ->where('id', '=', $userId)
        ->fetch('real_name'); // Assuming 'real_name' is the column name

    // Check if the author was found and return the real name, otherwise return null or a default value
    if ($author) {
        return $author->real_name;
    }

    // Return a default value if the author is not found (optional)
    return null;
}

function getAuthorAvatar($userId) {
    // Query the 'users' table to get the real name of the author based on the user ID
    $avatar = Query::table(Base::table('users'))
        ->where('id', '=', $userId)
        ->fetch('image'); // Assuming 'real_name' is the column name
